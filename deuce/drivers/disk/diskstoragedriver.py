...
class DiskStorageDriver(BlockStorageDriver):
    ...
    def _get_vault_path(self, storage_url, project_id, vault_id,
            auth_token=None):
        return os.path.join(
            self._path, str(project_id), vault_id)

    def _get_block_path(self, storage_url, project_id, vault_id, block_id,
            auth_token=None):
        vault_path = self._get_vault_path(storage_url, project_id, vault_id)
        return os.path.join(vault_path, str(block_id))

    def create_vault(self, storage_url, project_id, vault_id,
            auth_token=None):
        path = self._get_vault_path(storage_url, project_id, vault_id)
    ...
    def store_block(self, storage_url, project_id, vault_id, block_id,
            blockdata, auth_token=None):
        path = self._get_block_path(
            storage_url, project_id, vault_id, block_id)
